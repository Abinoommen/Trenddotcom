...
export default function Navbar() {
  return (
    <Disclosure
      as="nav"
      className="relative sticky top-0 z-50 bg-gray-800 dark:bg-gray-800/50 dark:after:pointer-events-none dark:after:absolute dark:after:inset-x-0 dark:after:bottom-0 dark:after:h-px dark:after:bg-white/10"
    >
        ...
        <div className="relative flex h-16 items-center justify-between">
          ...
          <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
            <div className="flex shrink-0 items-center">
              <p className="text-2xl font-bold text-white">Chati<span class="text-blue-500">Z</span></p>
            </div>
            <div className="hidden sm:ml-6 sm:block">
              <div className="flex space-x-4">
                {navigation.map((item) => (
                  <Link
                    key={item.name}
                    to={item.id}          // must match <Element name="...">
                    smooth={true}
                    spy={true}                 // watch scroll position
                    activeClass="nav-active"   // class when section is in view
                    duration={500}
                    offset={-80}
                    aria-current={item.id ? 'page' : undefined}
                    className='cursor-pointer text-gray-300 hover:bg-white/5 hover:text-white rounded-md px-3 py-2 text-sm font-medium'
                  >
                    {item.name}
                  </Link>
                ))}
